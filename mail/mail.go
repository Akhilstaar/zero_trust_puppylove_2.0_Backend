...
func SendMail(name string, to string, authCode string) error {
	...
	mailTemplate := fmt.Sprintf("<html><style>.container{font-size: large;}body{font-family: Cambria, Cochin, Georgia, Times, 'Times New Roman', serif;}code{padding: 6px;background-color: antiquewhite;border-radius: 5px;}</style><body><div class='container'>Hello %s,<br><br>This is your authentication code for Zero-trust-PuppyLove.<br><code>%s</code> <br><br>Thank you!</div></body></html>", name, authCode)
